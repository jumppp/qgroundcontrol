...
Item {
    ...
    function _addInitialPolygon() {
        if (_structurePolygon.count < 3) {
            ...
            var _circleRadius = _missionItem.sectorRadius.value
            var segments = 3
            var flyAngle = _missionItem.flyAngle.value
            var angleIncrement = flyAngle / segments
            var angle = _missionItem.startAngle.value
            _structurePolygon.clear()
            _structurePolygon.appendVertex(unboundCenter)
